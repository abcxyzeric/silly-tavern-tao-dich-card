...
export function generateId(): string {return crypto.randomUUID? crypto.randomUUID(): Date.now().toString(36) + Math.random().toString(36).slice(2);}

/** Full lorebook entry interface (shared across components, hooks, services) */
export interface LorebookEntry {id: string;
 name: string;
 keys: string[];
...
export interface WizardCharacter {id: string;
 name: string;
 description: string;
...
export interface AIGeneratedCharacter {name?: string;
 description?: string;}

/** AI parsed result for lorebook entry generation */
export interface AIGeneratedLorebookEntry {name?: string;
 keys?: string[];
 secondary_keys?: string[];
 content?: string;
 comment?: string;
...
export interface AIOrganizeSuggestion {index: number;
 position?: string;
 insertion_order?: number;
 depth?: number;
 probability?: number;
...
export interface AIGeneratedKeys {index: number;
 keys: string[];}
...
export interface MvuVariable {id: string;
 path: string[]; // e.g. ['Vai tròA', 'Sự thuận lợi']
...
export interface AIGeneratedMvuVariable {path: string[];
 kind: MvuVariableKind;
 defaultValue: unknown;
 description: string;
 enumValues?: string[];
...
export interface MvuConfig {enabled: boolean;
...
export interface WizardDraft {cardName: string;
...
export function createEmptyCharacter(): WizardCharacter {return {id: generateId(),
 name: '',
 description: '',};}
 ...
 */
export function createEmptyLorebookEntry(): LorebookEntry {return {
  // ── Core fields (V2 spec) ───────────────────────────────────────────────
 id: generateId(),
...
export const LOREBOOK_POSITION_OPTIONS = [{value: 'before_char', label: "Trước khi xác định vai trò", desc: "Ảnh hưởng vừa phải"},
 {value: 'after_char', label: "Sau khi xác định vai trò", desc: "Ảnh hưởng lớn hơn (được khuyến nghị)"},
 {value: 'before_example', label: "trước tin nhắn mẫu", desc: "được phân tích thành khối hội thoại"},
 {value: 'after_example', label: "Sau tin nhắn mẫu", desc: "được phân tích thành khối hội thoại"},
 {value: 'before_author', label: "Trước ghi chú của tác giả", desc: "Phụ thuộc vào vị trí AN"},
 {value: 'after_author', label: "Sau ghi chú của tác giả", desc: "Phụ thuộc vào vị trí AN"},
 {value: 'at_depth', label: "Chỉ định độ sâu", desc: "Chèn ở độ sâu tin nhắn được chỉ định"},] as const;

/** Secondary key logic modes (selectiveLogic) */
export const SELECTIVE_LOGIC_OPTIONS = [{value: 0, label: "VÀ BẤT CỨ", desc: "Được kích hoạt khi có bất kỳ từ lọc nào khớp"},
 {value: 1, label: "VÀ TẤT CẢ", desc: "Chỉ được kích hoạt khi tất cả các từ lọc khớp"},
 {value: 2, label: "KHÔNG TẤT CẢ", desc: "Kích hoạt khi ít nhất một cái không khớp"},
 {value: 3, label: "KHÔNG BẤT CỨ NÀO", desc: "Kích hoạt khi tất cả không khớp"},] as const;

/** Message role options */
export const LOREBOOK_ROLE_OPTIONS = [{value: 0, label: 'System', desc: "Thông báo hệ thống (mặc định)"},
 {value: 1, label: 'User', desc: "Tin nhắn của người dùng"},
 {value: 2, label: 'Assistant', desc: "tin tức AI"},] as const;

/**
 * Empty wizard draft state.
 * Includes all SillyTavern V2 spec fields.
 */
export function createEmptyDraft(): WizardDraft {return {cardName: '',
...
export function createEmptyMvuConfig(): MvuConfig {return {enabled: false,
 variables: [],
 schemaJs: '',
 initvarYaml: '',
 updateRulesYaml: '',
...
export const WIZARD_STEPS = [{id: 1, label: "Tên thẻ", required: true},
 {id: 2, label: "Cấu hình vai trò", required: true},
 {id: 3, label: "sổ thế giới", required: false},
 {id: 4, label: "phát biểu khai mạc", required: true},
 {id: 5, label: "Cuộc trò chuyện mẫu", required: false},
 {id: 6, label: "Làm đẹp/MVU", required: false},
 {id: 7, label: "Xem trước lưu", required: true},] as const;
