 ...
 */
const POSITION_INDEX: Record<string, number> = {before_char: 0, // before_character_definition
 ...
 */
const SELECTIVE_LOGIC_INDEX: Record<number, number> = {0: 0, // AND ANY → and_any
 1: 3, // AND ALL → and_all
 2: 1, // NOT ALL → not_all
 3: 2, // NOT ANY → not_any
};
...
const SELECTIVE_LOGIC_REVERSE: Record<number, number> = {0: 0, // and_any → AND ANY
 3: 1, // and_all → AND ALL
 1: 2, // not_all → NOT ALL
 2: 3, // not_any → NOT ANY
};
 ...
 */
function buildSTExtensions(overrides: {position: string;
 displayIndex: number;
 probability?: number;
 group?: string;
 groupWeight?: number;
 ...
 cooldown?: number;
 delay?: number;
 ignoreBudget?: boolean;} = {position: 'after_char',
 displayIndex: 0,}): Record<string, unknown> {return {position: POSITION_INDEX[overrides.position]?? 1,
 ...
 */
function buildCharacterEntry(charName: string, content: string, order: number) {return {id: 1000 + order,
 keys: [charName],
 secondary_keys: [],
 content,
 name: `${charName}- Cài đặt nhân vật`,
 enabled: true,
 ...
 position: 'after_char',
 priority: 100,
 comment: `${charName} thiết lập vai trò`,
 use_regex: false,
 extensions: buildSTExtensions({position: 'after_char',
 displayIndex: order,
...
export function assembleCard(draft: WizardDraft, existingId?: number) {
 ...
 const entries = draft.lorebookEntries.map((entry, i) => ({id: i + 1,
 keys: entry.keys,
 ...
 extensions: buildSTExtensions({position: entry.position?? 'after_char',
 displayIndex: i,
...
...(existingId? {id: existingId}: {}),
 ...
 _meta: {characters: draft.characters.map((c) => ({id: c.id || generateId(),
 name: c.name,
 ...
 */
export function exportAsJson(card: ReturnType<typeof assembleCard>) {const d = card.data;
 const exportObj = {
 ...
 */
export async function exportAsPng(card: ReturnType<typeof assembleCard>,
 pngBuffer?: ArrayBuffer,) {const {embedJsonInPng, downloadPng} = await import('./png-service');
 ...
 */
export async function importFromPng(pngBuffer: ArrayBuffer,): Promise<Record<string, unknown> | null> {const {extractJsonFromPng} = await import('./png-service');
 ...
 */
export function cardToDraft(card: Record<string, unknown>): WizardDraft {const data = (card.data || card) as Record<string, unknown>;
 ...
 if (meta.characters && Array.isArray(meta.characters) && (meta.characters as unknown[]).length > 0) {characters = (meta.characters as unknown[]).map((c: unknown) => {const ch = c as Record<string, unknown>;
 return {id: (ch.id as string) || generateId(),
 name: (ch.name as string) || '',
 description: (ch.description as string) || '',
 entryIds: (ch.entryIds as string[]) || [],};}) as WizardDraft['characters'];} else if (data.description) {
  // Fallback: single character from description
 characters = [{id: generateId(),
 ...
 if (characters.length === 0) {const generatedCharacterEntries = rawEntries.filter((e) => {const name = (e.name as string) || '';
 return e.constant === true && name.endsWith("- Cài đặt nhân vật") && typeof e.content === 'string';});

 characters = generatedCharacterEntries.map((e) => ({id: generateId(),
 name: ((e.name as string) || '').replace(/ - Cài đặt nhân vật$/, ''),
 description: (e.content as string) || '',
 entryIds: [(e.id as string) || generateId()],}));}

 return {cardName: (data.name as string) || (card.name as string) || '',
 characters,
 lorebookEntries: rawEntries.map((e, i) => {const ext = (e.extensions || {}) as Record<string, unknown>;
 return {id: (e.id as string) || generateId(),
