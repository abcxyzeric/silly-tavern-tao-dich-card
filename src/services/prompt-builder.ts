...
interface CardData {data: {name: string;
 description: string;
 personality: string;
 scenario: string;
 first_mes: string;
 ...
 */
export function buildSystemPrompt(card: CardData): string {const data = card.data;
 const charName = data.name || 'Character';

 // If the card has a system_prompt override, use it
 // (with {{original}} placeholder support)
 if (data.system_prompt?.trim()) {const defaultPrompt = buildDefaultSystemPrompt(card);
 ...
 */
function buildDefaultSystemPrompt(card: CardData): string {const data = card.data;
 const charName = data.name || 'Character';
 const sections: string[] = [];

 // 1. Character Description (permanent token - always sent)
 if (data.description?.trim()) {sections.push(data.description);}

 // 2. Personality summary (permanent token)
 if (data.personality?.trim()) {sections.push(`Personality: ${data.personality}`);}

 // 3. Scenario (permanent token)
 if (data.scenario?.trim()) {sections.push(`Scenario: ${data.scenario}`);}
 ...
 if (data.character_book?.entries) {const constantEntries = data.character_book.entries.filter((e) => e.constant && e.enabled).sort((a, b) => a.insertion_order - b.insertion_order);

 if (constantEntries.length > 0) {const worldInfo = constantEntries.map((e) => e.content).join('\n\n');
 ...
 const examples = data.mes_example.replace(/\{\{char\}\}/gi, charName).replace(/\{\{user\}\}/gi, 'You');
 sections.push(`Example conversations:\n ${examples}`);}

 // 6. Character instruction (stay in character)
 sections.push(`You are ${charName}. Stay in character at all times. ` +
 ...
 */
export function buildPostHistoryInstructions(card: CardData): string {const data = card.data;
