 ...
 */
export function applyBackground(imageUrl: string | null) {const bgImage = imageUrl || DEFAULT_BG;
 ...
 */
export function getStoredBackground(): string | null {return localStorage.getItem(STORAGE_KEY);}

/**
 * Store a new background image and apply it.
 */
export function setBackground(imageDataUrl: string): void {localStorage.setItem(STORAGE_KEY, imageDataUrl);
 applyBackground(imageDataUrl);}

/**
 * Clear the custom background and revert to default.
 */
export function clearBackground(): void {localStorage.removeItem(STORAGE_KEY);
 ...
 */
export function initBackground(): void {const stored = getStoredBackground();
 applyBackground(stored);}

/**
 * Convert a File to a data URL (base64).
 */
export function fileToDataUrl(file: File): Promise<string> {return new Promise((resolve, reject) => {const reader = new FileReader();
