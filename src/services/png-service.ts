...
function crc32(bytes: Uint8Array): number {let crc = 0xffffffff;
...
function deflateRaw(data: Uint8Array): Uint8Array {
 ...
 while (offset < data.length) {const remaining = data.length - offset;
 ...
 for (const block of blocks) {for (let i = 0; i < block.length; i++) {result.push(block[i]);}}
...
function adler32(data: Uint8Array): number {let a = 1, b = 0;
 for (let i = 0; i < data.length; i++) {a = (a + data[i]) % 65521;
 b = (b + a) % 65521;}
 return ((b << 16) | a) >>> 0;}

// ─── Make a PNG chunk ─────────────────────────────────────────────────────────
function makeChunk(type: string, data: Uint8Array): Uint8Array {const typeBytes = new TextEncoder().encode(type);
...
// ─── Read PNG chunks ──────────────────────────────────────────────────────────
interface PngChunk {type: string;
 data: Uint8Array;
 offset: number;}

function readPngChunks(buffer: ArrayBufferLike): PngChunk[] {const bytes = new Uint8Array(buffer);
 const view = new DataView(buffer);

 // Verify PNG signature
 for (let i = 0; i < 8; i++) {if (bytes[i]!== PNG_SIGNATURE[i]) {throw new Error("Không phải là tệp PNG hợp lệ");}}

 const chunks: PngChunk[] = [];
 let offset = 8;

 while (offset < bytes.length) {if (offset + 8 > bytes.length) break;
 ...
 */
export function embedJsonInPng(pngBuffer: ArrayBufferLike | null,
 cardJson: Record<string, unknown>,): Uint8Array {const jsonString = JSON.stringify(cardJson);
 ...
 for (const chunk of chunks) {if (chunk.type === 'IEND') {
 ...
 let offset = 0;
 for (const part of result) {output.set(part, offset);
 offset += part.length;}
 return output;}
 ...
 for (const chunk of chunks) {if (chunk.type === 'IEND' &&!textInserted) {result.push(makeChunk('tEXt', textData));
 textInserted = true;}
 result.push(makeChunk(chunk.type, chunk.data));}
 ...
 */
export function extractJsonFromPng(pngBuffer: ArrayBufferLike,): Record<string, unknown> | null {const chunks = readPngChunks(pngBuffer);

 for (const chunk of chunks) {if (chunk.type === 'tEXt') {const text = new TextDecoder().decode(chunk.data);
 ...
 if (keyword === 'chara') {try {const jsonString = decodeURIComponent(escape(atob(value)));
 return JSON.parse(jsonString);} catch {throw new Error("Phân tích cú pháp dữ liệu chara ở dạng PNG không thành công (định dạng base 64/JSON không hợp lệ)");}}}}
 ...
 */
export function downloadPng(pngData: Uint8Array, filename: string) {const blob = new Blob([pngData as BlobPart], {type: 'image/png'});
