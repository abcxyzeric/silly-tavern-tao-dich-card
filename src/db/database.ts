...
export interface CreatorChat {id?: number;
 title: string;
 messages: Array<{role: 'user' | 'assistant'; content: string}>;
 createdAt: Date;
 updatedAt: Date;}

export interface AISettings {id: number;
 apiUrl: string;
 apiKey: string;
 model: string;
 maxTokens: number;
...
export interface WizardDraftRecord {id: string;
 data: unknown;
 currentStep: number;
 updatedAt: Date;}

export const db = new Dexie('TavernCardHelper') as Dexie & {cards: EntityTable<CardRecord, 'id'>;
...
db.version(4).stores({cards: '++id, name, updatedAt, createdAt, deletedAt',
 ...
 */
export function maskApiKey(key: string): string {if (!key || key.length <= 8) return '••••••••';
 return key.slice(0, 3) + '••••' + key.slice(-4);}

/**
 * Get or create AI settings singleton.
 */
export async function getAISettings(): Promise<AISettings> {let settings = await db.ai_settings.get(1);
 ...
 */
export async function saveAISettings(settings: Partial<AISettings>): Promise<AISettings> {const current = await getAISettings();
