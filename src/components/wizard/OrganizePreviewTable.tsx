...
interface OrganizePreviewTableProps {entries: LorebookEntry[];
 suggestions: AIOrganizeSuggestion[];
 onApply: () => void;
 onDismiss: () => void;}

export function OrganizePreviewTable({entries,
 suggestions,
 onApply,
 onDismiss,}: OrganizePreviewTableProps) {return (<div className="mb-4 rounded-lg border border-amber-700/40 bg-slate-900/80 overflow-hidden">
 <div className="flex items-center justify-between px-4 py-2 bg-amber-900/20 border-b border-amber-700/30">
 <span className="text-sm font-semibold text-amber-300">⚡ Đề xuất sắp xếp AI ({suggestions.length} điều chỉnh)</span>
 <div className="flex gap-2">
 <Button size="sm" onClick={onApply}>✅Áp dụng tất cả</Button>
 <Button size="sm" variant="ghost" onClick={onDismiss}>Hủy bỏ</Button>
 </div>
 </div>
 ...
 <th className="text-left px-3 py-2">lối vào</th>
 <th className="text-left px-3 py-2">tham số</th>
 <th className="text-left px-3 py-2">giá trị hiện tại</th>
 <th className="text-left px-3 py-2">Giá trị đề xuất</th>
 <th className="text-left px-3 py-2">lý do</th>
 </tr>
 </thead>
 <tbody>
 {suggestions.map((r, i) => {const entry = entries[r.index];
 if (!entry) return null;
 return (<tr key={i} className="border-b border-slate-800/50 hover:bg-amber-900/10">
 <td className="px-3 py-1.5 font-medium text-slate-200 truncate max-w-[100px]">
 {entry.name || `lối vào ${r.index + 1}`}
 </td>
