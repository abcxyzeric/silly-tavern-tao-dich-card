...
interface AIGenerateButtonProps {/** Label displayed on the button */
 label?: string;
...
export function AIGenerateButton({label = "✨AI được tạo ra",
 hint = '',
 onGenerate,
 onAccept,}: AIGenerateButtonProps) {const [loading, setLoading] = useState(false);
 const [error, setError] = useState<string | null>(null);
 const [result, setResult] = useState<string | null>(null);

 const handleGenerate = async () => {setLoading(true);
 setError(null);
 try {const text = await onGenerate(hint);
 setResult(text);} catch (err: unknown) {setError(err instanceof Error? err.message: "Xây dựng không thành công");} finally {setLoading(false);}};
 ...
 <span className="animate-spin inline-block w-3 h-3 border-2 border-white/30 border-t-white rounded-full" />Đang tạo...</span>): (label)}
 </Button>
 ...
 <Modal isOpen={!!result} onClose={handleReject} title="Kết quả do AI tạo ra" maxWidth="max-w-2xl">
 ...
 <Button variant="ghost" onClick={handleReject}>Bỏ qua</Button>
 <Button variant="secondary" onClick={handleGenerate} disabled={loading}>
 {loading? "Tái sinh...": "tái sinh"}
 </Button>
 <Button onClick={handleAccept}>Áp dụng</Button>
 </div>
 </div>
 </Modal>
 </>);}
