...
import {CHARACTER_GENERATE_PROMPT,
 ...
 parseAIJson,} from '../constants/prompts';
import type {AIGeneratedCharacter,
 AIGeneratedLorebookEntry,
 AIOrganizeSuggestion,
 AIGeneratedKeys,
 AIGeneratedMvuVariable,} from '../constants/defaults';

export function useAIGenerate() {/**
 * Generate a character profile (non-streaming).
 * @returns Parsed character object or raw text if parse fails
 */
 const generateCharacter = useCallback(async (characterName: string, hint: string): Promise<string> => {const prompts = CHARACTER_GENERATE_PROMPT(characterName, hint);
 ...
 */
 const generateCharacterStreaming = useCallback(async (characterName: string,
 hint: string,
 onChunk: StreamCallback,): Promise<string> => {const prompts = CHARACTER_GENERATE_PROMPT(characterName, hint);
 ...
 */
 const generateCharacterParsed = useCallback(async (characterName: string, hint: string) => {const text = await generateCharacter(characterName, hint);
 const parsed = parseAIJson(text) as AIGeneratedCharacter | null;

 if (!parsed) return {description: text};

 return {name: parsed.name,
 description: parsed.description,};}, [generateCharacter]);
 ...
 */
 const generateCharacterParsedStreaming = useCallback(async (characterName: string,
 hint: string,
 onChunk: StreamCallback,) => {const text = await generateCharacterStreaming(characterName, hint, onChunk);
 const parsed = parseAIJson(text) as AIGeneratedCharacter | null;

 if (!parsed) return {description: text};

 return {name: parsed.name,
 description: parsed.description,};}, [generateCharacterStreaming]);

 /**
 * Generate lorebook skeleton entries in batch.
 * Returns ultra-compressed entries for fast iteration.
 */
 const generateLorebookSkeleton = useCallback(async (cardName: string,
 characterSummaries: string,
 topic: string,
 batchSize: number,
 existingTitles: string,
 rules?: string,): Promise<Array<{comment: string; content: string; keys: string[]; strategy: string}>> => {const prompts = LOREBOOK_SKELETON_PROMPT(cardName, characterSummaries, topic, batchSize, existingTitles, rules);
 ...
 return (parsed || []).map((sk) => ({comment: sk.comment || "Vô danh",
 content: sk.content || "(Sẽ được mở rộng)",
 ...
 */
 const generateLorebook = useCallback(async (cardName: string, characterSummaries: string, topic: string, rules?: string): Promise<string> => {const prompts = LOREBOOK_GENERATE_PROMPT(cardName, characterSummaries, topic, rules);
 ...
 */
 const generateLorebookStreaming = useCallback(async (cardName: string,
 characterSummaries: string,
 topic: string,
 onChunk: StreamCallback,
 rules?: string,): Promise<string> => {const prompts = LOREBOOK_GENERATE_PROMPT(cardName, characterSummaries, topic, rules);
 ...
 */
 const generateLorebookParsed = useCallback(async (cardName: string, characterSummaries: string, topic: string, rules?: string) => {const text = await generateLorebook(cardName, characterSummaries, topic, rules);
 const parsed = parseAIJson(text) as AIGeneratedLorebookEntry[] | null;
 return parsed || [];}, [generateLorebook]);

 /**
 * Generate lorebook entries with streaming and parse as JSON array.
 */
 const generateLorebookParsedStreaming = useCallback(async (cardName: string,
 characterSummaries: string,
 topic: string,
 onChunk: StreamCallback,
 rules?: string,) => {const text = await generateLorebookStreaming(cardName, characterSummaries, topic, onChunk, rules);
 const parsed = parseAIJson(text) as AIGeneratedLorebookEntry[] | null;
 return parsed || [];}, [generateLorebookStreaming]);

 /** Generate first message */
 const generateFirstMessage = useCallback(async (cardName: string,
 characterDescriptions: string,
 sceneHint: string,
 targetWordCount?: number,
 worldbookContext?: string,): Promise<string> => {const prompts = FIRST_MESSAGE_PROMPT(cardName, characterDescriptions, sceneHint, targetWordCount, worldbookContext);
 ...
 const generateFirstMessageStreaming = useCallback(async (cardName: string,
 characterDescriptions: string,
 sceneHint: string,
 onChunk: StreamCallback,
 targetWordCount?: number,
 ...
 const generateExampleDialogues = useCallback(async (cardName: string, characterDescriptions: string, worldbookContext?: string): Promise<string> => {const prompts = EXAMPLE_DIALOGUES_PROMPT(cardName, characterDescriptions, worldbookContext);
 ...
 const generateExampleDialoguesStreaming = useCallback(async (cardName: string,
 characterDescriptions: string,
 onChunk: StreamCallback,
 worldbookContext?: string,): Promise<string> => {const prompts = EXAMPLE_DIALOGUES_PROMPT(cardName, characterDescriptions, worldbookContext);
 ...
 * Reference: st-card-builder AITổ chức thông minh.
 */
 const organizeEntries = useCallback(async (entries: Array<{index: number;
 name: string;
 content: string;
 keys: string[];
 position: string;
 ...
 depth: number;
 probability: number;
 constant: boolean;}>) => {const prompts = ORGANIZE_ENTRIES_PROMPT(entries);
 ...
 * Reference: st-card-builder AITạo từ kích hoạt.
 */
 const generateEntryKeys = useCallback(async (entries: Array<{index: number;
 name: string;
 content: string;
 existingKeys: string[];}>) => {const prompts = GENERATE_KEYS_PROMPT(entries);
 ...
 */
 const generateMvuVariables = useCallback(async (cardName: string,
 characterSummaries: string,
 worldbookSummary: string,
 firstMessageExcerpt: string,) => {const prompts = MVU_VARIABLES_PROMPT(cardName, characterSummaries, worldbookSummary, firstMessageExcerpt);
 ...
 */
 const expandLorebookEntry = useCallback(async (entry: {comment: string;
 content: string;
 keys: string[];
 strategy: string;
 position: number;},
 characterContext: string,
 userRequirement?: string,) => {const isSkeleton = (entry.content || '').length < 120;
 ...
 return {comment: parsed?.comment?? entry.comment,
 ...
 */
 const translateCard = useCallback(async (cardData: Record<string, unknown>,
 targetLang: 'zh' | 'en',): Promise<Record<string, unknown> | null> => {
 ...
 for (const key of fields) {if (cardData[key] && typeof cardData[key] === 'string' && (cardData[key] as string).trim()) {translatable[key] = cardData[key];}}
 ...
 if (charBook?.entries && Array.isArray(charBook.entries)) {translatable._worldbook_entries = (charBook.entries as Array<Record<string, unknown>>).map(e => ({name: e.name || '',
 comment: e.comment || '',
 content: e.content || '',
 keys: e.keys || [],}));}
 ...
 */
 const diagnoseCard = useCallback(async (cardData: Record<string, unknown>,): Promise<{overall_score: number;
 summary: string;
 ...
 for (const key of fields) {if (cardData[key] && typeof cardData[key] === 'string') {
  // Truncate very long fields to save tokens
 const val = cardData[key] as string;
 diagContent[key] = val.length > 2000? val.slice(0, 2000) + "...(cắt ngắn)": val;}}

 // Include worldbook summary
 const charBook = cardData.character_book as Record<string, unknown> | undefined;
 if (charBook?.entries && Array.isArray(charBook.entries)) {diagContent._worldbook_count = (charBook.entries as unknown[]).length;
 diagContent._worldbook_entries = (charBook.entries as Array<Record<string, unknown>>).slice(0, 10).map(e => ({name: e.name || '',
 comment: e.comment || '',
